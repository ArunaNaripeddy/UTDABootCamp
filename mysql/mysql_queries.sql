...
ALTER TABLE 
  actor 
ADD 
  COLUMN middle_name VARCHAR(45) 
AFTER 
  first_name;

-- 3b. You realize that some of these actors have tremendously long last names. Change the data type of the middle_name column to blobs.
ALTER TABLE 
  actor MODIFY COLUMN middle_name BLOB 
AFTER 
  first_name;
...
SELECT 
    last_name, COUNT(last_name) AS 'Number of Actors'
FROM
    actor
GROUP BY last_name;
...
SELECT 
    ...
    CONCAT(YEAR(p.payment_date),'-',MONTH(p.payment_date)) AS 'Year-Month',
...
SELECT 
    f.title AS 'Title',
    COUNT(fa.actor_id) AS '# of Actors Acted'
FROM
    film f
        INNER JOIN
